...
async def run_sender(recipient_address: str):
    sender = Agent(name="Sender", port=8007, seed="sender_seed_phrase_123")

    # Ensure sender has funds to communicate on network if needed (fetch.ai testnet)
    ...
    async def say_hello(ctx: Context):
        ...
        # 2. Send Query
        query_text = "Do you have any summer clothing?"
    ...
    async def handle_response(ctx: Context, sender: str, msg: ChatMessage):
        for item in msg.content:
            if isinstance(item, TextContent):
                ctx.logger.info(f"Received Response: {item.text}")
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python sender.py <agent_address>")
        sys.exit(1)

    recipient = sys.argv[1]
    asyncio.run(run_sender(recipient))
